  ...
  for (MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi) {
    ...
    amrex::ParallelFor(tile_box, [=] AMREX_GPU_DEVICE(int i, int j,
      ...
      for (int i_part = 0; i_part < num_ppc; i_part++) {
        ...
        if (x >= a_bounds.hi(0) || x < a_bounds.lo(0) || y >= a_bounds.hi(1) ||
            y < a_bounds.lo(1) || z >= a_bounds.hi(2) || z < a_bounds.lo(2))
          continue;
...
extern "C" void TestParticles_Init(CCTK_ARGUMENTS) {
  ...
  amrex::RealBox a_bounds({AMREX_D_DECL(-20e-6, -20e-6, -20e-6)},
                          {AMREX_D_DECL(20e-6, 20e-6, 20e-6)});

  const array<const CCTK_REAL, 3> dx{CCTK_DELTA_SPACE(0), CCTK_DELTA_SPACE(1),
                                     CCTK_DELTA_SPACE(2)};

  const array<int, 3> a_num_particles_per_cell{4, 4, 4};
  const int num_ppc = a_num_particles_per_cell[0] *
                      a_num_particles_per_cell[1] * a_num_particles_per_cell[2];
  const Real scale_fac = dx[0] * dx[1] * dx[2] / num_ppc;
